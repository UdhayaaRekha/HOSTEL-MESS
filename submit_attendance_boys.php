...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Fetch all students
    $fetch_students_sql = "SELECT id, reg_no FROM user_management.boys_users";
    ...
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            ...
            $check_sql = "SELECT * FROM attendance_system.boys_attendance WHERE reg_no = ? AND date = ?";
            ...
            if ($result_check->num_rows > 0) {
                ...
                $update_sql = "UPDATE attendance_system.boys_attendance SET status = ? WHERE reg_no = ? AND date = ?";
                ...
                $insert_sql = "INSERT INTO attendance_system.boys_attendance (reg_no, date, status) VALUES (?, ?, ?)";
