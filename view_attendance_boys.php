...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "SELECT id, name FROM user_management.boys_users WHERE reg_no=?";
    ...
    if ($student_id) {
        ...
        $sql = "SELECT date, status FROM attendance_system.boys_attendance WHERE reg_no=? AND MONTH(date) = ? AND YEAR(date) = ?";
        $stmt = $conn->prepare($sql);
