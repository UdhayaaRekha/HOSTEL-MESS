...
<table>
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['current_status'])) {
        ...
        $update_sql = "UPDATE payments SET status='$current_status' WHERE reg_no='$reg_no' AND MONTH(payment_date) = '$selected_month' AND YEAR(payment_date) = '$selected_year'";
    ...
    if (isset($_POST['show_students']) && !empty($_POST['month']) && !empty($_POST['year'])) {
        ...
        $sql = "SELECT reg_no, name FROM users";
        ...
        if ($result->num_rows > 0) {
            while($row = $result->fetch_assoc()) {
                ...
                $reg_no = $row['reg_no'];
                $payment_sql = "SELECT receipt_path, status FROM payments WHERE reg_no='$reg_no' AND MONTH(payment_date) = '$selected_month' AND YEAR(payment_date) = '$selected_year'";
